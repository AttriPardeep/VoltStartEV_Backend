...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const chargers = await SteveService.getAvailableChargers({ lat: lat ? parseFloat(lat as string) : undefined, lng: lng ? parseFloat(lng as string) : undefined, minPower: minPower ? parseFloat(minPower as string) : undefined });
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (!charger) return errorResponse(res, 'CHARGER_NOT_FOUND', 'Charger not found', 404);
